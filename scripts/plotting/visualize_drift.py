...
plt.title(f"Global Model Weight Divergence (Client Drift) - {run_name}", fontsize=14, fontweight='bold')
plt.xlabel("Communication Round", fontsize=12)
plt.ylabel("L2 Norm of Global Update $||w^{(t)} - w^{(t-1)}||_2$", fontsize=12)
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()

os.makedirs("analysis_plots", exist_ok=True)
output_image = f"analysis_plots/{run_name}_global_weight_drift.png"
