...
plt.plot(X_pca[:, 0], X_pca[:, 1], '-o', color='royalblue', label=f'Caminho {run_name}', linewidth=1.5, markersize=5)
plt.scatter(X_pca[0, 0], X_pca[0, 1], color='green', s=150, zorder=5, label=f'Início (Rodada {valid_rounds[0]})')
plt.scatter(X_pca[-1, 0], X_pca[-1, 1], color='red', s=150, zorder=5, label=f'Fim (Rodada {valid_rounds[-1]})')
...
plt.title(f"Trajetória Global PCA - {run_name}", fontsize=14, fontweight='bold')
...
output_image = f"analysis_plots_pt/{run_name}_global_trajectory_pca.png"
